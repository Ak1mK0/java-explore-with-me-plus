...
1. Пользователь **{userId}** и мероприиятии **{eventId}** существуют. Иначе выбрасывается ошибка **NOT_FOUND**. (например *NotFoundException*)

2. Оценку можно постоавить мероприятию **{eventId}** которое уже состоялось. Иначе выбрасывается ошибка **BAD_REQUEST**. (например *ConditionsNotMetException*)

3. Пользователь **{userId}** был на мероприиятии **{eventId}**. Иначе выбрасывается ошибка **BAD_REQUEST**. (например *ConditionsNotMetException*)
...
Поставить *dislike* на мероприиятии **{eventId}**
...
Удалить *like/dislike* на мероприиятии **{eventId}**
...
##  4. Получить рейтинги мероприиятии

*Краткое описание:*

Получить рейтинги мероприиятии **{eventId}**
...
1. Мероприиятии **{eventId}** существуют. Иначе выбрасывается ошибка **NOT_FOUND**. (например *NotFoundException*)

##  5. Получить рейтинг мероприиятий пользователя

*Краткое описание:* 

Получить список мероприятий которым пользователь **{userId}** поставил оценку.
...
Получает список событий которые пользователь отметил *"like"*. Отсортированные в порядке убывания по времени
...
Получает список событий которые пользователь отметил *"dislike"*. Отсортированные в порядке убывания по времени
...
Получает список событий которые пользователь отметил. Отсортированные в порядке убывания по времени
...
2. Небязательность параметров **{rating}**, **{from}**, **{size}**
...
получить список мероприятий которым пользовати поставили наибольшую среднюю оценку **(сумма(*like) - сумма(dislike))**.
...
**order={order}** исползется для задания направления сортировки. По умолчанию используется *DESC*, сортировка по убыванию средней оценки. Так-же возможен вариант *ASC* - сортировка по возрастанию средней оценки.
...
1. Небязательность параметров **{from}**, **{size}**, **{order}** 